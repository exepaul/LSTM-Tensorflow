...
word_embedding = tf.get_variable('embedding',dtype=tf.float32,shape=[len(vocab_),150],initializer=tf.random_uniform_initializer(-0.01,0.01))

lookup=tf.nn.embedding_lookup(word_embedding,input_x)

sequ_lendth= tf.count_nonzero(input_x,axis=-1)

with tf.variable_scope('encoder') as scope:
    cell=rnn.LSTMCell(num_units=223)
    model=tf.nn.bidirectional_dynamic_rnn(cell,cell,inputs=lookup,sequence_length=sequ_lendth,dtype=tf.float32)
...
first1=tf.transpose(model_output[0],[1,0,2])
second=tf.transpose(model_output[1],[1,0,2])

concat2=tf.concat((first1[-1],second[-1]),axis=-1)


concat=tf.concat((fs.c,fc.c),axis=-1)

weights=tf.get_variable('weighvariable',shape=[2*223,len(labels_x)],dtype=tf.float32,initializer=tf.random_uniform_initializer(-0.01,0.01))

bias = tf.get_variable('bias',shape=[len(labels_x)],dtype=tf.float32,initializer=tf.random_uniform_initializer(-0.01,0.01))

final_result=tf.matmul(concat,weights)+bias


#normalization

normalalization=tf.nn.softmax(final_result)
pred=tf.argmax(normalalization,axis=-1)
...
with tf.Session() as sess:
    ...
    a,b,c,d,e,f,g=sess.run([lookup,model_output,first1,concat2,concat,final_result,fs],feed_dict={input_x:data_pad,output_y:labels_x})
    print("loo",a.shape)    #200x8x150
    print("model_output",b[0].shape)  #200x8x223
    print("transpose",c.shape)  #8x200x223
    print("concat",d.shape)      #200x446
    print("concat2",e.shape)     #200x446
    print("final_resut",f.shape)   #200x200

    print("each_cell",g.c.shape)  #200x223
