...
Wo = tf.get_variable('Wo',shape=[hdim * 2, num_labels],dtype=tf.float32,initializer=tf.random_uniform_initializer(-0.01, 0.01))

bo = tf.get_variable('bo',shape=[num_labels, ],dtype=tf.float32,initializer=tf.random_uniform_initializer(-0.01, 0.01))
...
with tf.Session() as sess:
    ...
    for i in range(500):
        aa,vv,cc,dd,ee,rr=sess.run([loss,probs,preds,logits,accuracy,trainop],feed_dict={name:data_pad,labels:labels_x})
